...
class TsuryPhoneNotificationManager:
    ...
    def __init__(
        self, hass: HomeAssistant, coordinator: TsuryPhoneDataUpdateCoordinator
    ):
    ...
    async def _handle_maintenance_mode_notification(
        ...
        if state.maintenance_mode:
            ...
            if not self._is_notification_active(NOTIFICATION_ID_MAINTENANCE):
                ...
                _LOGGER.info(
                    "Created maintenance mode notification for device %s",
                    self.device_id,
                )
        else:
            ...
            if self._is_notification_active(NOTIFICATION_ID_MAINTENANCE):
                ...
                _LOGGER.info(
                    "Dismissed maintenance mode notification for device %s",
                    self.device_id,
                )
        ...
        if not state.connected and state.last_seen:
            try:
                ...
                if last_seen_dt is None:
                    raise ValueError(
                        f"Unsupported last_seen value: {state.last_seen!r}"
                    )

                offline_duration = dt_util.utcnow() - last_seen_dt

                # Create notification if offline for more than 10 minutes
                if offline_duration > timedelta(
                    minutes=10
                ) and not self._is_notification_active(NOTIFICATION_ID_SYSTEM_ERROR):
                    ...
                    _LOGGER.warning(
                        "Created offline notification for device %s", self.device_id
                    )
            ...
            if (
                self._is_notification_active(NOTIFICATION_ID_SYSTEM_ERROR)
                and self._notification_states[NOTIFICATION_ID_SYSTEM_ERROR].get(
                    "message_type"
                )
                == "device_offline"
            ):
                ...
                _LOGGER.info(
                    "Dismissed offline notification for device %s", self.device_id
                )
    ...
    async def _handle_reboot_notification(self, state: TsuryPhoneState) -> None:
        ...
        if state.reboot_detected:
            ...
            last_reboot_notification = self._notification_states.get(
                NOTIFICATION_ID_REBOOT, {}
            )
            ...
            if should_notify:
                ...
                _LOGGER.info(
                    "Created reboot notification for device %s", self.device_id
                )

                # Auto-dismiss reboot notification after 30 seconds
                self.hass.async_create_task(
                    self._auto_dismiss_notification(
                        notification_id, NOTIFICATION_ID_REBOOT, 30
                    )
                )
    ...
    async def _handle_missed_calls_notification(self, state: TsuryPhoneState) -> None:
        ...
        if state.call_history:
            ...
            for call in state.call_history:
                if call.missed and call.timestamp and call.timestamp > cutoff_time:
        ...
        if missed_calls_count > last_notified_count and missed_calls_count > 0:
            ...
            if missed_calls_count == 1:
                ...
                caller_info = (
                    f"from {latest_call.name or latest_call.number}"
                    if latest_call
                    else ""
                )
                message = (
            ...
            _LOGGER.info(
                "Created/updated missed calls notification for device %s (%d calls)",
                self.device_id,
                missed_calls_count,
            )

        # Dismiss notification if no recent missed calls
        elif missed_calls_count == 0 and self._is_notification_active(
            NOTIFICATION_ID_MISSED_CALLS
        ):
            await self._persistent_notification_dismiss(notification_id)
            self._clear_notification_state(NOTIFICATION_ID_MISSED_CALLS)
            _LOGGER.info(
                "Dismissed missed calls notification for device %s", self.device_id
            )

    async def _auto_dismiss_notification(
        self, notification_id: str, notification_type: str, delay_seconds: int
    ) -> None:
        ...
        if self._is_notification_active(notification_type):
            ...
            _LOGGER.info(
                "Auto-dismissed %s notification for device %s",
                notification_type,
                self.device_id,
            )
    ...
    def _set_notification_active(
        self, notification_type: str, state_data: dict[str, Any]
    ) -> None:
...
async def async_setup_notifications(
    hass: HomeAssistant, coordinator: TsuryPhoneDataUpdateCoordinator
) -> TsuryPhoneNotificationManager:
...
async def async_unload_notifications(
    hass: HomeAssistant, coordinator: TsuryPhoneDataUpdateCoordinator
) -> None:
