...
async def _async_get_coordinator_diagnostics(
    ...
    return {
        ...
        "update_time": coordinator.last_update_time.isoformat() if coordinator.last_update_time else None,
        "update_interval": coordinator.update_interval.total_seconds(),
        "data_available": coordinator.data is not None,
        "websocket_connected": coordinator._websocket_client.connected if coordinator._websocket_client else False,
        ...
        "reboot_detected": coordinator.data.reboot_detected if coordinator.data else False,
    }
...
async def _async_get_device_specific_data(
    ...
    return {
        "device_state": {
            ...
            "previous_app_state": state.previous_app_state.value if state.previous_app_state else None,
        ...
        "call_state": {
            ...
            "current_call": {
                ...
                "start_time": state.current_call.start_time,
            ...
            "last_call": {
                ...
                "start_time": state.last_call.start_time,
        ...
        "lists": {
            ...
            "priority_count": state.priority_count if hasattr(state, 'priority_count') else len(getattr(state, 'priority_callers', []) or []),
...
async def _get_websocket_stats(coordinator: TsuryPhoneDataUpdateCoordinator) -> dict[str, Any]:
    ...
    return {
        ...
        "queue_size": getattr(websocket, '_message_queue_size', 0) if hasattr(websocket, '_message_queue_size') else 0,
    }


async def _get_api_client_stats(coordinator: TsuryPhoneDataUpdateCoordinator) -> dict[str, Any]:
...
def _get_integration_info(coordinator: TsuryPhoneDataUpdateCoordinator) -> dict[str, Any]:
    ...
    return {
        ...
        "services": [
            "dial", "answer", "hangup", "ring_device", "set_ring_pattern",
            "reset_device", "set_dnd", "set_audio", "get_call_history",
            "clear_call_history", "quick_dial_add", "quick_dial_remove",
            "quick_dial_clear", "blocked_add", "blocked_remove", "blocked_clear",
            "priority_add", "priority_remove",
            "refetch_all", "get_diagnostics", "webhook_add", "webhook_remove",
            "webhook_clear", "webhook_test", "switch_call_waiting",
            "set_maintenance_mode", "get_missed_calls", "quick_dial_import",
            "quick_dial_export", "blocked_import", "blocked_export",
        ],
        "features": {
...
def get_diagnostic_summary(coordinator: TsuryPhoneDataUpdateCoordinator) -> dict[str, Any]:
    ...
    health_factors = {
        ...
        "good_signal": state.stats.rssi_dbm > -80 if state.stats.rssi_dbm != 0 else None,
    }
    ...
    return {
        "status": "healthy" if health_percentage >= 80 else "warning" if health_percentage >= 60 else "error",
...
def _get_diagnostic_recommendations(
    ...
    if not state.connected:
        recommendations.append("Device is offline - check power and network connectivity")
    ...
    if coordinator._websocket_client and not coordinator._websocket_client.connected:
        recommendations.append("WebSocket disconnected - real-time updates may be delayed")
    
    if state.stats.rssi_dbm != 0 and state.stats.rssi_dbm < -80:
        recommendations.append("Poor WiFi signal strength - consider improving network coverage")
    
    if state.stats.free_heap_bytes < 10000:
        recommendations.append("Low device memory - consider reboot or reduced polling frequency")
    
    if state.maintenance_mode:
        recommendations.append("Device in maintenance mode - normal operations may be affected")
    
    recent_missed_calls = len([c for c in (state.call_history or []) if c.missed])
    if recent_missed_calls > 5:
        recommendations.append(f"{recent_missed_calls} recent missed calls - check DND settings and device availability")
