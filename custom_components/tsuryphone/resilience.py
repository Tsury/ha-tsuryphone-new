...
SEQUENCE_REBOOT_DETECTION_THRESHOLD = (
    100  # If sequence drops by more than this, likely a reboot
)
...
class TsuryPhoneResilience:
    ...
    async def handle_event_sequence(self, event: TsuryPhoneEvent) -> bool:
        ...
        if current_seq < self._last_sequence:
            ...
            if sequence_drop > SEQUENCE_REBOOT_DETECTION_THRESHOLD:
                _LOGGER.warning(
                    "Large sequence regression detected: %d -> %d (drop: %d)",
                    self._last_sequence,
                    current_seq,
                    sequence_drop,
                )
        ...
        # Check for sequence overflow
            ...
            current_seq > SEQUENCE_RESET_THRESHOLD
            and not self._sequence_overflow_detected
        ):
    ...
    async def _handle_potential_reboot(
        self, event: TsuryPhoneEvent, sequence_drop: int
    ) -> None:
        """Handle potential device reboot detection."""
        _LOGGER.info(
            "Potential device reboot detected (sequence drop: %d)", sequence_drop
        )
    ...
    async def _confirm_reboot_recovery(self) -> None:
        ...
        try:
            ...
            if (
                hasattr(self.coordinator, "_storage_cache")
                and self.coordinator._storage_cache
            ):
                try:
                    reboot_data = {
                        "reboot_time": dt_util.utcnow().isoformat(),
                        "sequence_drop": self._last_sequence
                        - (self.coordinator.data.last_seq or 0),
                        "pre_reboot_state": self._pre_reboot_state,
                    }
                    await self.coordinator._storage_cache.async_save_config_backup(
                        {"event_type": "reboot_recovery", "data": reboot_data}
                    )
                except Exception as err:
    ...
    def get_error_rate(self, error_type: str, window_minutes: int = 15) -> float:
        ...
        recent_errors = sum(
            1
            for timestamp in self._error_timestamps.values()
    ...
    def is_device_healthy(self) -> tuple[bool, list[str]]:
        ...
        # Check recent reboot
        if (
            self.stats.last_reboot_time
            and dt_util.utcnow() - self.stats.last_reboot_time < timedelta(minutes=5)
        ):
    ...
    def get_resilience_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "stats": {
                ...
                "last_reboot_time": (
                    self.stats.last_reboot_time.isoformat()
                    if self.stats.last_reboot_time
                    else None
                ),
                "last_sequence_reset": (
                    self.stats.last_sequence_reset.isoformat()
                    if self.stats.last_sequence_reset
                    else None
                ),
            },
    ...
    async def run_health_check(self) -> dict[str, Any]:
        ...
        websocket_healthy = (
            self.coordinator._websocket_client
            and self.coordinator._websocket_client.connected
        )
        health_results["checks"]["websocket_connectivity"] = {
            "status": "pass" if websocket_healthy else "fail",
            "message": (
                "WebSocket connected" if websocket_healthy else "WebSocket disconnected"
            ),
        }
        ...
        sequence_healthy = (
            not self._sequence_overflow_detected and self.stats.events_dropped < 5
        )
        ...
        # Error rate check
        high_error_rate = any(
            self.get_error_rate(error_type) > 1 for error_type in self._error_counts
        )
        health_results["checks"]["error_rate"] = {
            ...
            "message": (
                ...
                else "Elevated error rate detected"
            ),
        }
        ...
        recent_reboot = (
            self.stats.last_reboot_time
            and dt_util.utcnow() - self.stats.last_reboot_time < timedelta(minutes=2)
        )
        health_results["checks"]["reboot_status"] = {
            "status": "warning" if recent_reboot else "pass",
            "message": (
                "Recent reboot detected" if recent_reboot else "No recent reboots"
            ),
        }
