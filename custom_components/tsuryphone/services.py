...
SET_AUDIO_SCHEMA = vol.Schema({
    vol.Required("device_id"): cv.string,
    vol.Optional("earpiece_volume"): vol.All(vol.Coerce(int), vol.Range(min=AUDIO_MIN_LEVEL, max=AUDIO_MAX_LEVEL)),
    vol.Optional("earpiece_gain"): vol.All(vol.Coerce(int), vol.Range(min=AUDIO_MIN_LEVEL, max=AUDIO_MAX_LEVEL)),
    vol.Optional("speaker_volume"): vol.All(vol.Coerce(int), vol.Range(min=AUDIO_MIN_LEVEL, max=AUDIO_MAX_LEVEL)),
    vol.Optional("speaker_gain"): vol.All(vol.Coerce(int), vol.Range(min=AUDIO_MIN_LEVEL, max=AUDIO_MAX_LEVEL)),
})
...
BLOCKED_ADD_SCHEMA = vol.Schema({
    vol.Required("device_id"): cv.string,
    vol.Required("number"): cv.string,
    vol.Optional("reason"): cv.string,  # Changed from 'name' to 'reason' to match API
})
...
RESILIENCE_TEST_SCHEMA = vol.Schema({
    ...
    vol.Optional("test_type", default="connection"): vol.In(["connection", "sequence", "stress"]),
})
...
async def async_resilience_test(call: ServiceCall) -> ServiceResponse:
    ...
    if test_type == "connection":
        ...
        try:
            ...
            test_results["results"].append({"test": "api_connectivity", "status": "pass"})
        except Exception as err:
            test_results["results"].append({"test": "api_connectivity", "status": "fail", "error": str(err)})
...
async def async_setup_services(hass: HomeAssistant) -> None:
    ...
    async def async_get_coordinator(device_id: str) -> TsuryPhoneDataUpdateCoordinator:
        ...
        raise ServiceValidationError(f"TsuryPhone device with ID '{device_id}' not found")
    ...
    async def async_set_audio(call: ServiceCall) -> None:
        ...
        audio_config = {}
        for key in ["earpiece_volume", "earpiece_gain", "speaker_volume", "speaker_gain"]:
            if key in call.data:
                # Convert to camelCase for API
                api_key = key.replace("_", "").replace("earpiece", "earpiece").replace("speaker", "speaker")
    ...
    async def async_clear_call_history(call: ServiceCall) -> None:
        ...
        if older_than_days or keep_last:
            ...
            if older_than_days:
                ...
                coordinator.data.call_history = [
                    entry for entry in coordinator.data.call_history 
                    if entry.received_ts > cutoff
                ]
            
            if keep_last and len(coordinator.data.call_history) > keep_last:
                coordinator.data.call_history = coordinator.data.call_history[-keep_last:]
        else:
            # Clear all
            coordinator.data.call_history = []
        ...
        if coordinator._storage_cache:
            await coordinator._storage_cache.async_save_call_history(coordinator.data.call_history)
    ...
    async def async_quick_dial_clear(call: ServiceCall) -> None:
        ...
        if errors:
            raise HomeAssistantError(f"Some entries failed to clear: {'; '.join(errors)}")
    ...
    async def async_get_tsuryphone_config(call: ServiceCall) -> dict[str, Any]:
        ...
        try:
            ...
            raise HomeAssistantError(f"Failed to fetch TsuryPhone config: {err}") from err
    ...
    async def async_priority_remove(call: ServiceCall) -> None:
        ...
        try:
            ...
            raise HomeAssistantError(f"Failed to remove priority caller: {err}") from err
    ...
    async def async_dial_quick_dial(call: ServiceCall) -> None:
        ...
        try:
            ...
            raise HomeAssistantError(f"Failed to dial quick dial code {code}: {err}") from err
        ...
        try:
            ...
            for entry in entries:
                try:
                    ...
                    _LOGGER.warning("Failed to add quick dial entry %s: %s", entry["code"], err)
            ...
            raise HomeAssistantError(f"Failed to import quick dial entries: {err}") from err
    ...
    async def async_quick_dial_export(call: ServiceCall) -> dict[str, Any]:
        ...
        try:
            ...
            raise HomeAssistantError(f"Failed to export quick dial entries: {err}") from err
        ...
        try:
            ...
            for entry in entries:
                try:
                    ...
                    _LOGGER.warning("Failed to add blocked number %s: %s", entry["number"], err)
            ...
            raise HomeAssistantError(f"Failed to import blocked numbers: {err}") from err
    ...
    async def async_blocked_export(call: ServiceCall) -> dict[str, Any]:
        ...
        try:
            ...
            raise HomeAssistantError(f"Failed to export blocked numbers: {err}") from err
    ...
    services_config = [
        ...
        (SERVICE_CLEAR_CALL_HISTORY, async_clear_call_history, CALL_HISTORY_CLEAR_SCHEMA),
    ...
    (SERVICE_PRIORITY_REMOVE, async_priority_remove, PRIORITY_REMOVE_SCHEMA),
        ...
        (SERVICE_SET_MAINTENANCE_MODE, async_set_maintenance_mode, MAINTENANCE_MODE_SCHEMA),
    ...
    for service_name, service_func, schema in services_config:
        ...
        supports_response = SupportsResponse.OPTIONAL if service_name in [
            SERVICE_GET_CALL_HISTORY, SERVICE_GET_DIAGNOSTICS, SERVICE_GET_MISSED_CALLS,
            SERVICE_QUICK_DIAL_IMPORT, SERVICE_QUICK_DIAL_EXPORT, 
            SERVICE_BLOCKED_IMPORT, SERVICE_BLOCKED_EXPORT,
            # Phase P8: Resilience services with responses
            SERVICE_RESILIENCE_STATUS, SERVICE_RESILIENCE_TEST, 
            SERVICE_WEBSOCKET_RECONNECT, SERVICE_RUN_HEALTH_CHECK
...
async def async_unload_services(hass: HomeAssistant) -> None:
    ...
    services_to_remove = [
        SERVICE_DIAL, SERVICE_ANSWER, SERVICE_HANGUP, SERVICE_RING_DEVICE,
        SERVICE_SET_RING_PATTERN, SERVICE_RESET_DEVICE, SERVICE_SET_DND,
        SERVICE_SET_AUDIO, SERVICE_GET_CALL_HISTORY, SERVICE_CLEAR_CALL_HISTORY,
        SERVICE_QUICK_DIAL_ADD, SERVICE_QUICK_DIAL_REMOVE, SERVICE_QUICK_DIAL_CLEAR,
        SERVICE_BLOCKED_ADD, SERVICE_BLOCKED_REMOVE, SERVICE_BLOCKED_CLEAR,
        SERVICE_REFETCH_ALL, SERVICE_GET_DIAGNOSTICS, SERVICE_WEBHOOK_ADD,
        SERVICE_WEBHOOK_REMOVE, SERVICE_WEBHOOK_CLEAR, SERVICE_WEBHOOK_TEST,
        SERVICE_SWITCH_CALL_WAITING, SERVICE_SET_MAINTENANCE_MODE, SERVICE_GET_MISSED_CALLS,
        SERVICE_QUICK_DIAL_IMPORT, SERVICE_QUICK_DIAL_EXPORT,
        SERVICE_BLOCKED_IMPORT, SERVICE_BLOCKED_EXPORT,
        # Phase P8: Resilience services
        SERVICE_RESILIENCE_STATUS, SERVICE_RESILIENCE_TEST,
